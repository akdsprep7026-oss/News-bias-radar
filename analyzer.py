...
def load_summarizer():
    ...
    # We are explicitly providing the model name to ensure the task is recognized
    return pipeline(
        "summarization", 
...
def analyze_bias_advanced(text_to_analyze):
    ...
    # Defining core "seed" concepts for each ideology.
    ...
    # Getting the vector representations for our seed words
    ...
    # Creating an average "ideology vector" for each side
    left_ideology_vector = np.mean(left_vectors, axis=0)
    right_ideology_vector = np.mean(right_vectors, axis=0)

    # Processing the article text
    doc = nlp(text_to_analyze)
    
    # Only considering relevant words (nouns, adjectives, verbs)
    ...
    for token in relevant_words:
        ...
        # Calculating how similar each word is to the core ideologies
        ...
        # Checking which ideology the word is more similar to
        if left_sim > right_sim:
            left_similarity_scores.append(left_sim)
        else:
            right_similarity_scores.append(right_sim)

    # Final score is the average similarity to each ideology
    ...
    # Returning these as percentages for easier interpretation
